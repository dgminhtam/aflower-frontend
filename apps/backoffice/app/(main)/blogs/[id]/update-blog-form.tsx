...
export const updateBlogSchema = z.object({
    title: z.string().min(1, "Tiêu đề không được để trống").max(255, "Tiêu đề quá dài").optional(),
    slug: z.string().min(1, "Slug không được để trống").max(255)
        .regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/, "Slug chỉ được chứa chữ thường, số và dấu gạch ngang").optional(),
    content: z.string().min(1, "Nội dung không được để trống").optional(),
    shortDescription: z.string().optional(),
    thumbnailId: z.number().min(1, "Vui lòng chọn ảnh đại diện").optional(),
...
export function UpdateBlogForm({ blog }: UpdateBlogFormProps) {
    ...
    const form = useForm<UpdateBlogRequest>({
        ...
        defaultValues: {
            ...
            thumbnailId: blog.thumbnail?.id || 0,
            ...
            <Controller
                ...
                render={({ field, fieldState }) => (
                    <Field data-invalid={fieldState.invalid}>
                        ...
                        <GalleryUpload
                            initialMedia={blog.thumbnail ? [blog.thumbnail] : []}
                            onChange={(mediaIds) => {
